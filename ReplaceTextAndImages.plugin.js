...
module.exports = class ReplaceTextAndImages {
    start() {
        this.replaceContent();
    ...
    replaceContent() {
        ...
        document.querySelectorAll('*').forEach(elem => {
            ...
            Object.entries(imageReplacements).forEach(([oldUrl, newUrl]) => {
                if (style.backgroundImage.includes(oldUrl)) {
                    elem.style.backgroundImage = `url(${newUrl})`;
                }
            });
        });

        // Replace links (href attributes)
        document.querySelectorAll('a').forEach(link => {
            if (linkReplacements[link.href]) {
                link.href = linkReplacements[link.href];
            }
        });

        // Adjust specific element style
        const targetElement = document.querySelector("#app-mount > div.appAsidePanelWrapper_a3002d > div.notAppAsidePanel_a3002d > div:nth-child(4) > div:nth-child(2) > div");
        if (targetElement) {
            const style = targetElement.style;
            if (style.position === "absolute" && style.bottom === "860px" && style.left === "1962px") {
                style.left = "1995px";
            }
        }
    }
    ...
    observeMutations() {
        ...
        // Check periodically in case of missed mutations
        this.interval = setInterval(() => this.replaceContent(), 500);
    }
};